...
var Log = func() Logs {
	...
	ml := &mylog{
		BeeLogger: logs.NewLogger(LOG_CAP, LOG_FEEDBACK_LEVEL),
	}

	// 是否打印行信息
	ml.BeeLogger.EnableFuncCallDepth(LOG_LINEINFO)
	// 全局日志打印级别（亦是日志文件输出级别）
	ml.BeeLogger.SetLevel(LOG_LEVEL)
	// 是否异步输出日志
	ml.BeeLogger.Async(LOG_ASYNC)
	// 设置日志显示位置
